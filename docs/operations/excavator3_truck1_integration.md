...
Use separate terminals for the router, perception, each physical robot, and the
Command Center.
...
### Terminal 2: Perception on ROS PC
...
ros2 launch construction_robot_perception \
...
Excavator 3 is the default enabled swing adapter. Other excavators can be
enabled with launch arguments such as `excavator1:=true`.

Keep this terminal running.

### Terminal 3: Excavator03 Raspberry Pi
...
### Terminal 4: Dumptruck1 Raspberry Pi
...
source /opt/ros/jazzy/setup.bash
source install/setup.bash
source network/setup_zenoh.sh client truck1
...
### Terminal 5: Command Center on ROS PC

Start this terminal only after the physical workspace is clear and all
preflight checks below have passed.
...
`start_camera:=false` and `start_apriltag:=false` do not stop the running
perception system. They prevent the Command Center from starting duplicate
camera and AprilTag nodes. Terminal 2 must remain active.
...
Use the dedicated perception launch and keep the Command Center camera and
AprilTag launch arguments set to `false`.
