...
class SwingPositionAdapter(Node):
    ...
    def __init__(self):
        ...
        self.tag_frame = str(
            self.get_parameter('tag_frame').value
        ).lstrip('/')
        ...
        print_rate_hz = max(
            0.1,
            float(self.get_parameter('print_rate_hz').value),
        )
        ...
        self.get_logger().info(
            ...
            f'offset {math.degrees(self.camera_to_site_yaw_offset_rad):+.2f} '
            'deg; no automatic zeroing.'
        )
