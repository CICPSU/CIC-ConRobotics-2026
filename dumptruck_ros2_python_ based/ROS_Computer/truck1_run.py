...
default_waypoints = f"{yaml_folder}/truck1_waypoints.yaml"
...
print(f"Using waypoint YAML: {waypoints_yaml}")
...
-p odom_topic:=/truck1/odom \
-p odom_frame:=truck1/odom \
-p base_frame:=truck1/base_link \
-p publish_tf:=false""",

    f"""source /opt/ros/jazzy/setup.bash && cd {code_folder} && python3 tag_odom_fusion_node_landmarks_v4.py --ros-args \
-p odom_topic:=/truck1/odom \
-p fused_odom_topic:=/truck1/fused_odom \
-p robot_tag_child_frame:=tag36h11_0 \
-p fused_child_frame:=truck1/base_link_fused \
-p alpha:=0.03 \
...
-p odom_topic:=/truck1/fused_odom \
