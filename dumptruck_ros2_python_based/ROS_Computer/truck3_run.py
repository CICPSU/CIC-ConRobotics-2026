...
def get_waypoints_yaml() -> Path:
    ...
    return yaml_folder / "truck3_waypoints.yaml"
...
commands = [
    (
        ...
        f"-p wheel_states_topic:=/truck3/wheel_states "
        f"-p odom_topic:=/truck3/odom "
        f"-p odom_frame:=truck3/odom "
        f"-p base_frame:=truck3/base_link "
    ...
    (
        ...
        f"-p odom_topic:=/truck3/odom "
        f"-p fused_odom_topic:=/truck3/fused_odom "
        f"-p robot_tag_child_frame:=tag36h11_2 "
        f"-p fused_child_frame:=truck3/base_link_fused "
        f"-p alpha:=0.25 "
    ...
    (
        ...
        f"-p odom_topic:=/truck3/fused_odom "
        f"-p cmd_vel_topic:=/truck3/cmd_vel "
        f"-p bucket_action_cmd_topic:=/truck3/bucket_action_cmd "
        f"-p bucket_action_status_topic:=/truck3/bucket_action_status "
