...
- Chain extraction (base_link -> tip_link)
- Approximate planar link lengths from joint origins (X-Z plane)
- Basic link discovery / candidate resolution for URDF variants

This version is kept generic, but adds a couple of helpers so client code can
survive URDF revisions such as switching from bucket_end_link to bucket_link.
"""
...
class RobotModel:
    ...
    def __init__(self, joints: Dict[str, JointInfo], links: Set[str]) -> None:
        self.joints: Dict[str, JointInfo] = joints
        self.links: Set[str] = set(links)
    ...
    def from_urdf_file(path: str) -> "RobotModel":
        ...
        for j in root.findall("joint"):
            ...
            if axis_el is not None:
                axis = _parse_floats(axis_el.get("xyz"), 3, 0.0)  # type: ignore[arg-type]

            origin_el = j.find("origin")
            origin_xyz = (0.0, 0.0, 0.0)
            origin_rpy = (0.0, 0.0, 0.0)
            if origin_el is not None:
                origin_xyz = _parse_floats(origin_el.get("xyz"), 3, 0.0)  # type: ignore[arg-type]
                origin_rpy = _parse_floats(origin_el.get("rpy"), 3, 0.0)  # type: ignore[arg-type]
        ...
        return RobotModel(joints=joints, links=links)
    ...
    def resolve_link(self, preferred: str, fallbacks: Optional[List[str]] = None) -> str:
        if preferred in self.links:
            return preferred
        for fb in (fallbacks or []):
            if fb in self.links:
                return fb
        raise KeyError(
            f"Requested link '{preferred}' not found. Available links include: {sorted(self.links)}"
        )
    ...
    def chain_joints(self, base_link: str, tip_link: str) -> List[str]:
        ...
        while cur != base_link:
            ...
            if cur not in self._child_to_joint:
                raise KeyError(
                    f"No joint produces child link '{cur}'. Cannot build chain from '{base_link}' to '{tip_link}'."
                )
